...
public class ChatWebsocketHandler extends TextWebSocketHandler {
	...
	// -> 멀티스레드환경에서 하나의 컬렉션 요소에 여러스레드가 동시에 접근하면 충돌이 발생할 수 있으므로 동기화를 진행(줄세우기)
	...
	// 클라이언트와 연결이 종료되면 실행되는 메소드
	@Override
	public void afterConnectionClosed(WebSocketSession session, CloseStatus closeStatus) throws Exception{
		sessions.remove(session);
		// 웹소켓 연결이 종료된 경우, sessions안에 저장되어있던 클라이언트의 session정보를 삭제
	}
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception{
		...
		// payLoad : 전송되는 테이터(json객체)
		System.out.println("전달된 메세지 : "+message.getPayload());
		
		// Jackson라이브러리 : java에서 json을 다루기 위한 라이브러리(gson이랑 비슷함)
		// Jackson-databind -> ObjectMapper를 이용해서 JSON형태로 넘어온 데이터를 특정 VO필드에 맞게 자동 매핑시켜줌
		ObjectMapper objectMapper = new ObjectMapper();
		ChatMessage chatMessage = objectMapper.readValue(message.getPayload(), ChatMessage.class);
		String msg = chatMessage.getMessage().replace("\n", "<br>");
