...
public class BoardServiceImpl implements BoardService {
	 ...
	 * service, service내부의 코드 수행시점에서 에러 발생시 자동 rollback이 됨
	 * 
	 * 방법 1) aop를 활용하는 방법(현재는 안됨)
	 * 
	 * 방법 2) @Transactional어노테이션 활용하여 선언적 트랜잭션 처리
	 * 		  -> RuntimeException에 대한 처리를 기본값으로 가진다.
	 * 
	 * rollbackFor : rollback을 수행하기 위한 예외의 종류를 작성하는 부분
	 */
	@Transactional(rollbackFor= {Exception.class}) // 모든 종류의 예외가 발생하면 rollback시키겠다고 선언
		...
		if(boardNo > 0 && list != null) {
			...
			if(!boardImageList.isEmpty()) {
				...
				if(result == boardImageList.size()) {
					...
					// 서버에 이미지 저장(Utils.saveFile에서 선작업함)
				}else {
			...
			if(list != null) {
				...
				if(result > 0) {
					...
					for(BoardImg img : boardImgList) {
						...
						// 결과값 1 -> 수정작업이 잘 이뤄졌다 => 기존에 이미지가 있었다.
						// 결과값 0 -> 수정작업이 실패함 => 기존에 이미지가 없었다.
						
						// 6) 결과값이 0 => update에는 실패했고, 실제 db에는 올라가야 할 데이터이기 때문에 insert 시켜준다
						if(result==0) {
							result = boardDao.insertBoardImg(img);
							// -> 값을 하나씩 대입해서 삽입하는 경우 결과가 0이 나올 수 없음(단, 예외는 발생할 수 있음)
						}
					}
				}
	...
	public List<String> selectAllBoardImgs() {
		return boardDao.selectAllBoardImgs();
	}
