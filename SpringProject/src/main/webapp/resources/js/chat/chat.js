 ...
 */
 ...
 (); // 익명함수 곧바로 실행시킴 (즉시실행함수 IIFE) => 속도가 빠르고, 변수명 중복문제 해결
 ...
 function sendMessage(){
 	...
 	if(inputChatting.value.trim().length == 0){
 		...
 		inputChatting.value = ""; // 채팅 서버로 전송 후 입력창에서 지워주기
 	}
 }
 ...
 chattingSock.onmessage = function(e){
 	...
 	// 내가 쓴 채팅 : span -> p
 	// 남이 쓴 채팅 : p -> span
 	if(userNo == chatMessage.userNo){ // 내가 쓴 거
 		li.append(span, p);
 ...
 // 현재 시간 출력 함수 (createDate있지만 얘가 더 정확해서)
